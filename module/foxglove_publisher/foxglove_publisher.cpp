...
struct FoxglovePublisher::Impl {
    ...
    Impl(const std::string& config_file) {
        ...
        options.callbacks.onGetParameters = [this](
            uint32_t client_id, 
        ...
        if (!server_result.has_value()) {
            spdlog::error("Failed to create Foxglove server: {}", foxglove::strerror(server_result.error()));
    ...
    foxglove::RawChannel* getOrCreateChannel(
        ...
        const std::string& schema_encoding = "jsonschema") 
    {
        ...
        // 创建新 Channel
        auto channels_result = foxglove::RawChannel::create(
            topic,
            encoding,
            schema,
            context
...
// 占位函数实现，后续填充
void FoxglovePublisher::startRecording(const std::string &file_path) { (void)file_path; }
...
void FoxglovePublisher::publishImage(const cv::Mat &image, const std::string &topic)
{
    ...
    if (it == pImpl_->image_channels.end()) {
        ...
        if (!channel_res.has_value()) {
            spdlog::error("Failed to create image channel '{}': {}", topic, foxglove::strerror(channel_res.error()));
            return;
        }
...
void FoxglovePublisher::publishArmorDetection(const std::vector<basic_armor::Armor_Data> &armors) { (void)armors; }
void FoxglovePublisher::publishTracking(const predictor::Target &target) { (void)target; }
void FoxglovePublisher::publishSerialData(const nlohmann::json& serial_data)
{
    if (!pImpl_->server) return;
    ...
    auto* channel = pImpl_->getOrCreateChannel(
        "serial_data",
        "json",
        "",  // 不使用 schema name
        ""   // 不使用 schema text
...
void FoxglovePublisher::publishTransform(const std::string &parent_frame, const std::string &child_frame,
...
void FoxglovePublisher::publishPerformanceMetrics(const nlohmann::json &metrics) { (void)metrics; }
void FoxglovePublisher::setParameterCallback(ParameterCallback cb) { pImpl_->param_callback = cb; }
