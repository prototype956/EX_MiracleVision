...
// --------------------------- 视频捕获初始化 ---------------------------
  ...
  // DNN (深度神经网络) 模块初始化
  // onnx_inferring::model model_ = onnx_inferring::model(fmt::format("{}{}", SOURCE_PATH, "/module/ml/mnist-8.onnx")); // 示例代码，未启用
  ...
  session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL); // 设置图优化级别
  ...
  while (true)
  {
    ...
    cv::Size frameSize(cap_.get(cv::CAP_PROP_FRAME_WIDTH), cap_.get(cv::CAP_PROP_FRAME_HEIGHT)); // 视频尺寸
