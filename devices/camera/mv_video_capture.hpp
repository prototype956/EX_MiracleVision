...
inline auto idntifier_green =
    fmt::format(fg(fmt::color::green) | fmt::emphasis::bold, "mv_video_capture");
inline auto idntifier_red =
    fmt::format(fg(fmt::color::red) | fmt::emphasis::bold, "mv_video_capture");
...
class VideoCapture {
 public:
   ...
   * @return int
   */
  int cameraInit(const int _CAMERA_RESOLUTION_COLS, const int _CAMERA_RESOLUTION_ROWS,
