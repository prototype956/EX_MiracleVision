  ...
  if (img && x >= 0 && x < img->cols && y >= 0 && y < img->rows) {
    ...
    if (event == cv::EVENT_MOUSEMOVE) {
      ...
      if (std::abs(x - last_x) > 10 || std::abs(y - last_y) > 10) {
        fmt::print("\r(x={}, y={}) : R:{} G:{} B:{}    ", x, y, pixel[2], pixel[1], pixel[0]);
        std::cout << std::flush;
        last_x = x;
        last_y = y;
      }
...
int main(int argc, char** argv) {
  ...
  uart::SerialPort serial_ =
      uart::SerialPort(fmt::format("{}/serial/uart_serial_config.xml", CONFIG_FILE_PATH));

  // 基础装甲板检测模块
  basic_armor::Detector basic_armor_ =
      basic_armor::Detector(fmt::format("{}/armor/basic_armor_config.xml", CONFIG_FILE_PATH));
    ...
    try {
      ...
      if (name == "bullet_speed" && value.is_number()) {
        ...
        if (c == "BLUE")
          ...
          mock_data.my_color = uart::Color::RED;
      } else if (name == "run_mode" && value.is_string()) {
        std::string m = value.get<std::string>();
        if (m == "AUTO_AIM")
          mock_data.now_run_mode = uart::RunMode::AUTO_AIM;
        else if (m == "ENERGY_BUFF")
          mock_data.now_run_mode = uart::RunMode::ENERGY_BUFF;
        else if (m == "DEFAULT_MODE")
          mock_data.now_run_mode = uart::RunMode::DEFAULT_MODE;
      }
  ...
  while (current_video_idx < video_files.size()) {
    ...
    fmt::print("[Playing] ({}/{}) {}\n", current_video_idx + 1, video_files.size(), current_video);
    ...
    fmt::print("[Info] FPS: {:.2f}, 分辨率: {}x{}, 总帧数: {}\n", cap_fps, frameSize.width,
               frameSize.height, total_frames);
    ...
    while (!video_finished) {
      ...
      // 关键: 将模拟的欧拉角转换为四元数，并传递给 Solver 初始化云台坐标系
      {
        ...
        // 欧拉角转四元数 (ZYX顺序)
        Eigen::Quaterniond q_init = Eigen::AngleAxisd(yaw_rad, Eigen::Vector3d::UnitZ()) *
      ...
      if (detected) {
        ...
        for (int i = 0; i < armor_num; i++) {
          ...
          // Armor(int class_id, float confidence, const cv::Rect & box, std::vector<cv::Point2f>
          // armor_keypoints)
          predictor::Armor pred_armor(0,                          // class_id (暂时设为0)
                                      0.9f,                       // confidence
          ...
          tools::draw_text(src_img, label, cv::Point(armor_rect.center.x + 10, armor_rect.center.y),
      ...
      for (auto& armor : predictor_armors) {
        ...
        fmt::print(
            "[Armor {}] PnP yaw after optimize: {:.4f}rad ({:.1f}°) | xyz: ({:.2f}, {:.2f}, "
            "{:.2f})\n",
            armor_idx, armor.ypr_in_world[0], armor.ypr_in_world[0] * 57.3, armor.xyz_in_world[0],
      ...
      // 绘制追踪器状态
      tools::draw_text(src_img, fmt::format("[{}]", tracker.state()), {10, 180}, {255, 255, 255},
                       0.8, 2);
      ...
      if (!targets.empty()) {
        ...
        for (const auto& target : targets) {
          ...
          // 绘制车体中心（紫色十字）
          if (center_2d.x >= 0 && center_2d.x < src_img.cols && center_2d.y >= 0 &&
              center_2d.y < src_img.rows) {
            cv::drawMarker(src_img, center_2d, cv::Scalar(255, 0, 255), cv::MARKER_CROSS, 20, 3);
            tools::draw_text(src_img, "Center", cv::Point(center_2d.x + 15, center_2d.y - 15),
          ...
          for (const Eigen::Vector4d& xyza : armor_xyza_list) {
            ...
            // angle = 车体yaw + 装甲板相位偏移
            fmt::print(
                "  [Plate {}] angle(yaw+phase): {:.4f}rad ({:.1f}°) | xyz: ({:.2f}, {:.2f}, "
                "{:.2f})\n",
                plate_idx, xyza[3], xyza[3] * 57.3, xyza[0], xyza[1], xyza[2]);

            // 使用 solver 将 3D 坐标反投影回图像平面
            auto image_points = solver.reproject_armor(xyza.head(3),  // xyz 位置
          ...
          std::string target_info =
              fmt::format("ID:{} | x:{:.2f} y:{:.2f} z:{:.2f} | r:{:.2f} w:{:.2f}", target.last_id,
                          x[0], x[2], x[4], x[8], x[7]);
          tools::draw_text(src_img, target_info, cv::Point(10, 210), cv::Scalar(255, 255, 255), 0.5,
                           2);

          // 输出预测状态到终端
          fmt::print(
              "[Predictor] State:{} | ID:{} | xyz:({:.2f},{:.2f},{:.2f}) | r:{:.2f} w:{:.2f}\n",
      ...
      // 1. 绘制帧号
      tools::draw_text(src_img, fmt::format("[{}]", frame_count), cv::Point(10, 30),
                       cv::Scalar(255, 255, 255), 0.8, 2);

      // 2. 绘制视频信息
      tools::draw_text(src_img,
                       fmt::format("Video: {}/{} | FPS: {:.1f}", current_video_idx + 1,
                                   video_files.size(), cap_fps),
                       cv::Point(10, 60), cv::Scalar(154, 50, 205),  // 紫色
                       0.6, 2);

      // 3. 绘制装甲板检测信息
      tools::draw_text(src_img, fmt::format("Armors: {} | Targets: {}", armor_num, targets.size()),
                       cv::Point(10, 90), cv::Scalar(0, 255, 0),  // 绿色
                       0.6, 2);

      // 4. 绘制追踪状态
      tools::draw_text(src_img, fmt::format("Tracker: {}", tracker.state()), cv::Point(10, 120),
      ...
      tools::draw_text(
          src_img,
          fmt::format("Time: {:.2f}s / {:.2f}s", frame_count / cap_fps, total_frames / cap_fps),
          cv::Point(10, 150), cv::Scalar(0, 255, 255),  // 黄色
          0.6, 2);
      ...
      auto elapsed_ms =
          std::chrono::duration_cast<std::chrono::milliseconds>(now - last_foxglove_publish_time)
              .count();
      ...
      cv::putText(display_img,
                  fmt::format("Gimbal: Yaw={:.1f} Pitch={:.1f}", current_mock_data.yaw,
                              current_mock_data.pitch),
                  cv::Point(10, display_img.rows - 30), cv::FONT_HERSHEY_SIMPLEX, 0.6,
